...
static void gpt_irq_ack(const os_counter_t *dev, uint8_t chan_id, bool disable_irq)
{
...
static void gpt_irq_handler(void *irq_dev)
{
	...
	if (alarm->flags & OS_COUNTER_ALARM_CFG_LOOP) {
		gpt_irq_ack(dev, chan_id, false);
		next = alarm->last_ticks + alarm->ticks;
		alarm->last_ticks = next;
		GPT_SetOutputCompareValue((GPT_Type *)dev->base, kGPT_OutputCompare_Channel1, next);
	} else
		gpt_irq_ack(dev, chan_id, true);
		...
		const struct os_counter_alarm_cfg *alarm_cfg)
{
	...
	if (alarm_cfg->flags & OS_COUNTER_ALARM_CFG_LOOP) {
		alarm->last_ticks = next;
		alarm->ticks = alarm_cfg->ticks;
	}
