...
out:
		...
		if (virtio_queue_must_notify(vq)) {
			if (pdMS_TO_TICKS(VNET_COAL_TIME) == 0 || tx_cnt >= VNET_COAL_CNT) {
				ndev->vdev.ops->interrupt_front(&ndev->vdev, true);
				tx_cnt = 0;
			} else if (xTimerIsTimerActive(ndev->kick_timer) != pdTRUE) {
				xTimerChangePeriod(ndev->kick_timer, pdMS_TO_TICKS(VNET_COAL_TIME), 0);
			}
		}
	}
}
...
static void kick_timer_callback(TimerHandle_t timer)
{
	struct net_dev *ndev = (struct net_dev *)pvTimerGetTimerID(timer);
...
static int virtio_net_dev_init(struct net_dev *ndev, void *mmio_base)
{
	...
	ndev->kick_timer = xTimerCreate("kick_timer", 1, pdFALSE, ( void *  )ndev, kick_timer_callback);
	if (!ndev->kick_timer)
	{
		os_printf("%s: failed to create kick_timer\r\n", __func__);
		return -1;
	}
...
static int virtio_net_rx_callback(struct switch_port *port, void *data_paket)
{
	...
	if (virtio_queue_must_notify(vq)) {
		if (pdMS_TO_TICKS(VNET_COAL_TIME) == 0 || rx_cnt >= VNET_COAL_CNT) {
			ndev->vdev.ops->interrupt_front(&ndev->vdev, true);
			rx_cnt = 0;
		} else if (xTimerIsTimerActive(ndev->kick_timer) != pdTRUE) {
			xTimerChangePeriod(ndev->kick_timer, pdMS_TO_TICKS(VNET_COAL_TIME), 0);
...
static int virtio_net_port_add(struct net_dev *ndev)
{
	...
	strncpy(config.name, "VirtIO_Switch_Port", sizeof(config.name) - 1);
