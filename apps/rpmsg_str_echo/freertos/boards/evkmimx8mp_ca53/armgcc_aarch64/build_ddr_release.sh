...
do
	...
	do
		...
		if [ $each_console = "RAM_CONSOLE" ]; then
			...
			mv ddr_release/rpmsg_str_echo_ca53_RTOS${each_rtos}_${each_console}.bin ddr_release/rpmsg_str_echo_ca53_RTOS${each_rtos}_${each_console}-${buf_addr}.bin
			mv ddr_release/rpmsg_str_echo_ca53_RTOS${each_rtos}_${each_console}.elf ddr_release/rpmsg_str_echo_ca53_RTOS${each_rtos}_${each_console}-${buf_addr}.elf
		fi
	done
done
