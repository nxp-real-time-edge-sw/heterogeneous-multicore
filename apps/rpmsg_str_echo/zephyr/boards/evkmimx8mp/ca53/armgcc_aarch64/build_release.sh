...
do
	...
	do
		...
		west build -p always -b imx8mp_evk/mimx8ml8/a53 ${app_path} -DRTOS_ID=${each_rtos} -DCONSOLE=${each_console} -DCMAKE_BUILD_TYPE=${build_type} -d ${BUILD_DIR}
		if [ $? -ne 0 ]; then
			exit 1
		fi
		...
		ln -s rpmsg_str_echo.bin ${BUILD_DIR}/zephyr/rpmsg_str_echo_ca53_RTOS${each_rtos}_${console_name}.bin
		ln -s rpmsg_str_echo.elf ${BUILD_DIR}/zephyr/rpmsg_str_echo_ca53_RTOS${each_rtos}_${console_name}.elf
	done
done
