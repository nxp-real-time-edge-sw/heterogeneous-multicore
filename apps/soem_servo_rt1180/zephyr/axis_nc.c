...
inline static int skip_semicolon(char **str)
{
	...
	if (!p) {
		log_err("Each instruction must end with ';'\r\n");
		return -1;
	}
	*str = p + 1;
...
static int parameter_parser(char **str, char *parameter_name, int32_t *parameter)
{
	...
	if (strncmp(parameter_name, s, len) == 0) {
		...
		if (n != 1) {
			log_err("The parameter \"%s\" can not be assigned\r\n", parameter_name);
			return -1;
		}
...
/* Using trapezoidal move profile */
...
static int Tp_arrays_parser(struct axis_t *axis, char *tp_str, uint32_t period_ns)
{
	...
	if (axis->mode != op_mode_csp) {
		return -AXIS_PARSER_ERR_MODE_ERROR;
	}
	...
	while (*p != '\0') {
		...
		if (ret < 0)
			return -AXIS_PARSER_ERR_CYCLIC_ERROR;
		else if ( ret == 1) {
		...
		if (ret < 0)
			return -AXIS_PARSER_ERR_SCALE_ERROR;
		else if ( ret == 1) {
			axis->scale = value;
			continue;
		}
		...
		if (ret < 0)
			return -AXIS_PARSER_ERR_BIAS_ERROR;
		else if ( ret == 1) {
			status->bias = value;
			continue;
		}
		...
		if (ret < 0)
			return -AXIS_PARSER_ERR_ACCEL_ERROR;
		else if ( ret == 1) {
		...
		if (ret < 0)
			return -AXIS_PARSER_ERR_DECEL_ERROR;
		else if ( ret == 1) {
		...
		if (ret < 0)
			return -AXIS_PARSER_ERR_MAX_SPEED_ERROR;
		else if ( ret == 1) {
		...
		if (strncmp("TpArrays", p, 8) == 0) {
			...
			tp_arrays = strchr(p, '[');
			if (!tp_arrays) {
				return -AXIS_PARSER_ERR_PARSE_ERROR;
			}
			char *pp = strchr(p, ']');
			if (!pp) {
				return -AXIS_PARSER_ERR_PARSE_ERROR;
	...
	if (n == 0) {
		return -AXIS_PARSER_ERR_NO_TRAJECTORY;
	}

	if (n >= TRAJECTORY_PLANNER_MAX) {
		return -AXIS_PARSER_ERR_TOO_MANY_POINTS;
	}
