...
do
	...
	if [ $each_console = "RAM_CONSOLE" ]; then
		...
		mv ddr_release/rpmsg_pingpong_remote_ca53_${each_console}.bin ddr_release/rpmsg_pingpong_remote_ca53_${each_console}-${buf_addr}.bin
		mv ddr_release/rpmsg_pingpong_remote_ca53_${each_console}.elf ddr_release/rpmsg_pingpong_remote_ca53_${each_console}-${buf_addr}.elf
	fi
done
