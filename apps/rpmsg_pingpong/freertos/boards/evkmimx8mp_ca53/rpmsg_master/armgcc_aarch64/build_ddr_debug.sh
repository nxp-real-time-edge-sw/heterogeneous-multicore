...
do
	...
	if [ $each_console = "RAM_CONSOLE" ]; then
		...
		mv ddr_debug/rpmsg_pingpong_master_ca53_${each_console}.bin ddr_debug/rpmsg_pingpong_master_ca53_${each_console}-${buf_addr}.bin
		mv ddr_debug/rpmsg_pingpong_master_ca53_${each_console}.elf ddr_debug/rpmsg_pingpong_master_ca53_${each_console}-${buf_addr}.elf
	fi
done
