...
console=("RAM_CONSOLE" "UART4")

for i in "${!name_str[@]}"
do
        for j in "${!console[@]}"
        do
                output_dir=${build_target}_${name_str[$i]}_rtos${i}_${console[$j]}
                rm -rf ${output_dir}
                mkdir ${output_dir}
                ...
                west sdk_build -p always ${app_root_path} -b ${board} --config ${build_target} -Dcore_id=${cpu} -DCONSOLE=${console[$j]} -DRTOS_ID=$i -DIS_MASTER=${name_str[$i]} -d ${output_dir} -DCONFIG_SRAM_BASE_ADDRESS=${sram_addr}
                if [ $? -ne 0 ]; then
                        exit 1
                fi
